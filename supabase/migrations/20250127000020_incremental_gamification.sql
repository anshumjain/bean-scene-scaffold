-- Incremental Gamification Setup
-- This migration adds only the essential gamification features to existing schema
...
-- Create cafe_visits table if it doesn't exist
...
CREATE INDEX IF NOT EXISTS idx_user_stats_device_id ON public.user_stats(device_id);
...
CREATE INDEX IF NOT EXISTS idx_user_badges_device_id ON public.user_badges(device_id);
CREATE INDEX IF NOT EXISTS idx_user_badges_username ON public.user_badges(username);
CREATE INDEX IF NOT EXISTS idx_user_badges_badge_type ON public.user_badges(badge_type);

CREATE INDEX IF NOT EXISTS idx_cafe_visits_user_id ON public.cafe_visits(user_id);
CREATE INDEX IF NOT EXISTS idx_cafe_visits_device_id ON public.cafe_visits(device_id);
CREATE INDEX IF NOT EXISTS idx_cafe_visits_username ON public.cafe_visits(username);
CREATE INDEX IF NOT EXISTS idx_cafe_visits_cafe_id ON public.cafe_visits(cafe_id);
...
DROP POLICY IF EXISTS "Anyone can update user stats" ON public.user_stats;
...
DROP POLICY IF EXISTS "Anyone can view cafe visits" ON public.cafe_visits;
DROP POLICY IF EXISTS "Anyone can insert cafe visits" ON public.cafe_visits;
DROP POLICY IF EXISTS "Anyone can update cafe visits" ON public.cafe_visits;
...
-- Create RLS Policies for cafe_visits
